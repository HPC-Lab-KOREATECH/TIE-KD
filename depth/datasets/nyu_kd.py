...
class NYUDataset_kd(Dataset):
    ...
    def __init__(self,
        ...
        if self.data_root is not None:
            ...
            if not (self.kd_dir is None or osp.isabs(self.kd_dir)):
                self.kd_dir = osp.join(self.data_root, self.kd_dir)
    ...
    def load_annotations(self, data_root, split):
        ...
        if split is not None:
            with open(split) as f:
                for line in f:
                    ...
                    #################
                    img_info['kd'] = dict(depth_map=osp.join(data_root, remove_leading_slash(depth_map)))
                    # img_info['kd'] = dict(kd_map=depth_map)
                    #################
                    ...
                    # img_info['kd'] = osp.join(data_root, remove_leading_slash(img_name))
        ...
        # path_temp = img_infos[0]['kd']['depth_map']
        # my_list = path_temp.split('/')
        # path_temp = self.kd_dir + '/data_nyu_'+ my_list[2] +'_rgb_'+my_list[3].split('_')[2]
        # kd_map = path_temp 
        # kd_map_gt = np.asarray(Image.open(kd_map), dtype=np.
        # float32)          

        # print(np.max(kd_map_gt))
        # print(np.min(kd_map_gt))
        # sys.exit()
    ...
    def prepare_train_img(self, idx):
        ...
        kd_info = self.get_kd_info(idx)    
        # results = dict(img_info=img_info, ann_info=ann_info)
        results = dict(img_info=img_info, ann_info=ann_info, kd_info = kd_info)
    ...
    def get_kd_gt_depth_maps(self):
        """Get ground truth depth maps for evaluation."""
        for img_info in self.img_infos:                        
            path_temp = img_info['kd']['depth_map']
            my_list = path_temp.split('/')
            path_temp = self.kd_dir + '/data_nyu_'+ my_list[2] +'_rgb_'+my_list[3].split('_')[2]

            kd_map_gt = np.asarray(Image.open(path_temp), dtype=np.float32)/65525*10
            yield kd_map_gt
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            path_temp = self.img_infos[index]['kd']['depth_map']
            my_list = path_temp.split('/')
            path_temp = self.kd_dir + '/data_nyu_'+ my_list[2] +'_rgb_'+my_list[3].split('_')[2]
            kd_map = path_temp 
            kd_map_gt = np.asarray(Image.open(kd_map), dtype=np.float32)/65525*10    
            # kd_depth_map_gt = self.eval_kb_crop(kd_map_gt)                        
            kd_map_gt = np.expand_dims(kd_map_gt, axis=0)

            # print("@@@@@@@@@@@@@@")
            # print(depth_map_gt.shape)
            # print(valid_mask.shape)
            # print(kd_map_gt.shape)
            # valid_mask2 = self.eval_mask(kd_map_gt)
            # print(valid_mask2.shape)
            # sys.exit()




            # test_image = kd_map_gt/10*255                    
            # test_image = test_image.astype('uint8')
            # kd_img = Image.fromarray(test_image[0])
            # kd_img.save('kd_map_gt.png')

            # test_image = pred/10*255                    
            # test_image = test_image.astype('uint8')
            # kd_img = Image.fromarray(test_image[0])
            # kd_img.save('pred.png')

            # print(np.max(kd_map_gt))
            # print(np.min(kd_map_gt))
            # print(np.max(depth_map_gt))
            # print(np.min(depth_map_gt))
            # print(np.max(pred))
            # print(np.min(pred))
            # print("@@@@@@@@@@@@@@")

            eval = metrics(kd_map_gt,pred, depth_map_gt[valid_mask], pred[valid_mask], self.min_depth, self.max_depth)
    ...
    def evaluate(self, results, metric='eigen', logger=None, **kwargs):
        ...
        if mmcv.is_list_of(results, np.ndarray) or mmcv.is_list_of(
            ...
            kd_gt_depth_maps = self.get_kd_gt_depth_maps()            

            # print(gt_depth_maps.shape)
            # print(torch.max(gt_depth_maps))
            # print(torch.min(gt_depth_maps))
            # print(kd_gt_depth_maps.shape)
            # print(torch.max(kd_gt_depth_maps))
            # print(torch.min(kd_gt_depth_maps))
            # print("@@@@@@@@@@@@@@@@")
            # sys.exit()

            ret_metrics = eval_metrics(kd_gt_depth_maps, gt_depth_maps,
                results,self.min_depth,self.max_depth)
