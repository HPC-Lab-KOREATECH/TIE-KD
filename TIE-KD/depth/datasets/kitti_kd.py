...
class KITTIDataset_kd(Dataset):
    ...
    def load_annotations(self, img_dir, ann_dir, split):
        ...
        # path_temp = img_infos[0]['kd']['kd_map'][0:11]+img_infos[0]['kd']['kd_map'].replace('/','_')[0:27]+img_infos[0]['kd']['kd_map'].replace('/','_')[50:58]+'_data_'+img_infos[0]['kd']['kd_map'].replace('/','_')[59:]
        # print(path_temp)
        # print(self.kd_dir)
        # sys.exit()
    ...
    def get_kd_gt_depth_maps(self):
        """Get ground truth depth maps for evaluation."""
        for img_info in self.img_infos:
            path_temp = self.img_info['kd']['kd_map'][0:11]+self.img_info['kd']['kd_map'].replace('/','_')[0:27]+self.img_info['kd']['kd_map'].replace('/','_')[50:58]+'_data_'+self.img_info['kd']['kd_map'].replace('/','_')[59:]            
            kd_map = osp.join(self.kd_dir,
                               path_temp)
            kd_map_gt = np.asarray(Image.open(kd_map), dtype=np.float32)  
            yield kd_map_gt
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            ############################
            path_temp = self.img_infos[index]['kd']['kd_map'][0:11]+self.img_infos[index]['kd']['kd_map'].replace('/','_')[0:27]+self.img_infos[index]['kd']['kd_map'].replace('/','_')[50:58]+'_data_'+self.img_infos[index]['kd']['kd_map'].replace('/','_')[59:]            
            kd_map = osp.join(self.kd_dir,
                               path_temp)
            ...
            # if i == 0:
            #     print("hi")
            #     kd_gt_copy = kd_depth_map_gt/80*255        
            #     # kd_gt_copy=kd_gt_copy.cpu().detach().numpy()
            #     kd_gt_copy=kd_gt_copy.squeeze()
            #     kd_gt_copy = kd_gt_copy.astype('uint8')
            #     kd_img = Image.fromarray(kd_gt_copy)
            #     kd_img.save('kd_depth_map_gt.png')

            #     kd_gt_copy = pred/80*255        
            #     # kd_gt_copy=kd_gt_copy.cpu().detach().numpy()
            #     kd_gt_copy=kd_gt_copy.squeeze()
            #     kd_gt_copy = kd_gt_copy.astype('uint8')
            #     kd_img = Image.fromarray(kd_gt_copy)
            ...
            #     diff = abs(kd_depth_map_gt-pred)
            #     # print(np.max(diff))
            #     # sys.exit()
            #     kd_gt_copy = diff/36*255
            #     # kd_gt_copy=kd_gt_copy.cpu().detach().numpy()
            #     kd_gt_copy=kd_gt_copy.squeeze()
            #     kd_gt_copy = kd_gt_copy.astype('uint8')
            #     kd_img = Image.fromarray(kd_gt_copy)
            ...
            # sys.exit()
            ##############################
            eval = metrics(kd_depth_map_gt, pred, depth_map_gt[valid_mask], 
                           pred[valid_mask], 
                           min_depth=self.min_depth,
                           max_depth=self.max_depth)

            # eval = metrics(kd_depth_map_gt, pred, kd_depth_map_gt, 
            #                pred, 
            #                min_depth=self.min_depth,
            #                max_depth=self.max_depth)
            ############################
    ...
    def evaluate(self, results, metric='eigen', logger=None, **kwargs):
        ...
        num_table = len(ret_metrics) // 16
        for i in range(num_table):
            names = ret_metric_names[i*16: i*16 + 16]
            values = ret_metric_values[i*16: i*16 + 16]

        # num_table = len(ret_metrics) // 12
        # for i in range(num_table):
        #     names = ret_metric_names[i*12: i*12 + 12]
        #     values = ret_metric_values[i*12: i*12 + 12]
